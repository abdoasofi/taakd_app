...
const getDefaultState = (): VerificationRequestStoreState => ({
  ...
  step1: {
    ...
    phone: [] as PhoneNumber[], // أرقام الهاتف
...
export const useVerificationRequestStore = defineStore('verificationRequest', {
  ...
  actions: {
    ...
    // إجراءات لمعالجة PhoneNumbers
    addPhoneNumber(newPhone: PhoneNumber ) {
      this.step1.phone.push(newPhone);
    },

    removePhoneNumber(index: number) {
      const removed = this.step1.phone.splice(index, 1);
    },
    ...
    validatePhoneNumbers(): boolean {
      ...
      this.step1.phone.forEach((phone, index) => {
        if (!phone.phone || phone.phone.trim() === '') {
          isValid = false;
          this.updatePhoneNumber(index, { isValid: false, validationMessage: 'رقم الهاتف مطلوب.' });
        } else {
          this.updatePhoneNumber(index, { isValid: true, validationMessage: '' });
        }
      });
      return isValid;
    },

    updatePhoneNumber(index: number, value: Partial<PhoneNumber>) {
      if (this.step1.phone[index]) {
        this.step1.phone[index] = { ...this.step1.phone[index], ...value };
      }
    },

    // إجراءات لمعالجة AliasNames
    addAliasName(alias: AliasName) {
      this.step1.alias_name.push(alias);
    },

    removeAliasName(index: number) {
      const removed = this.step1.alias_name.splice(index, 1);
    },

    updateAliasName(index: number, value: Partial<AliasName>) {
      if (this.step1.alias_name[index]) {
        this.step1.alias_name[index] = { ...this.step1.alias_name[index], ...value };
      }
    },
    ...
    async loadStep1Fields() {
      try {
        ...
        this.validations = data.validations || this.validations;
      } catch (error) {
        const toast = useToast();
    ...
    async loadStep3Fields() {
      try {
        ...
        this.step3.employment_history = data.employment_history.map((employment: any) => ({
          employment_id: employment.employment_id || uuidv4(),
          company: employment.company || '',
          name_of_your_employer: employment.name_of_your_employer || '',
