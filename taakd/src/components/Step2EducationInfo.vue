...
<template>
  <div class="bg-white p-4 py-6 shadow-sm rounded-sm">
    ...
    <div v-if="educationInformation.length > 0" class="[&>*:nth-child(5n)]:break-before-page [&>*:nth-child(5n)]:py-8 py-5">
      ...
      >
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 gap-6">
          <InfoRow :label="$t('school_or_university')" :value="education.name_of_school_or_college_university" />
          <InfoRow :label="$t('field_of_study')" :value="education.field_of_study_or_major" />
          <InfoRow :label="$t('country')" :value="education.country" />
          <InfoRow :label="$t('city')" :value="education.city" />
          <InfoRow :label="$t('from_date')" :value="education.from_date" />
          <InfoRow :label="$t('to_date')" :value="education.to_date" />
        </div>
      </div>
    </div>
    ...
    <div v-else>
      <p class="text-gray-600">{{ $t('no_education_info') }}</p>
    </div>
  </div>
</template>
